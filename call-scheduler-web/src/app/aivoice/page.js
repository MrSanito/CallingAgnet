      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-bold text-slate-800 text-sm">Top AI Flagged Issues</h2>
              ...
              <div className="space-y-4">
                {issuesBreakdown.map((issue, i) => (
                  <div key={i}>
                    <div className="flex justify-between text-xs font-semibold mb-1.5">
                      <span className="text-slate-700 flex items-center gap-1.5">{issue.label}</span>
                      <span className="text-slate-500">{issue.text}</span>
                    </div>
                    <div className="w-full bg-slate-100 rounded-full h-2">
                      <div className={`h-2 rounded-full ${issue.color}`} style={{ width: `${issue.pct}%` }}></div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs">
              <span className="text-slate-500">Auto-prioritization via AI summaries is enabled.</span>
              <Link href="/aialerts" className="text-indigo-600 hover:text-indigo-800 font-bold hover:underline">
                Manage Flags &rarr;
              </Link>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-bold text-slate-800 text-sm">Distributor Performance</h2>
                <span className="text-[10px] text-slate-400 font-medium">Issue Rate</span>
              </div>
              <div className="overflow-x-auto">
                <table className="table table-xs w-full text-xs text-left">
                  <thead>
                    <tr className="text-slate-400 border-b border-slate-100">
                      <th className="pb-2 font-semibold">Distributor</th>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filteredDistributors.map((dist, i) => (
                      <tr key={i} className="hover:bg-slate-50/50">
                        <td className="py-2.5 font-medium text-slate-700">{dist.name}</td>
                        <td className="py-2.5 text-slate-600">{dist.calls}</td>
                        <td className="py-2.5 text-slate-600">{dist.issues}</td>
                        <td className={`py-2.5 text-right font-bold ${dist.color}`}>{dist.rate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-bold text-slate-800 text-sm">Issues by State</h2>
                <span className="text-[10px] text-slate-400 font-medium">Territory Rate</span>
              </div>
              <div className="overflow-x-auto">
                <table className="table table-xs w-full text-xs text-left">
                  <thead>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filteredStates.map((state, i) => (
                      <tr key={i} className="hover:bg-slate-50/50">
                        <td className="py-2 font-medium text-slate-700">{state.name}</td>
                        <td className="py-2 text-slate-600">{state.calls}</td>
                        <td className="py-2 text-slate-600">{state.issues}</td>
                        <td className={`py-2 text-right font-bold ${state.color}`}>{state.rate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-bold text-slate-800 text-sm">Language Mix</h2>
                <span className="text-[10px] text-slate-400 font-medium">Voice Eng.</span>
              </div>
              <div className="relative h-32 flex items-center justify-center">
                <canvas ref={languageChartRef}></canvas>
              </div>
              
              <div className="grid grid-cols-2 gap-1 mt-4 text-[10px]">
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded bg-indigo-600"></span>
                  <span className="text-slate-600">Hindi: 60%</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded bg-emerald-500"></span>
                  <span className="text-slate-600">English: 25%</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded bg-amber-400"></span>
                  <span className="text-slate-600">Marathi: 10%</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded bg-slate-300"></span>
                  <span className="text-slate-600">Others: 5%</span>
                </div>
              </div>
            </div>
          </div>

          {/* Retry & Followup Queue */}
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm flex flex-col justify-between">
            <div>
              <h2 className="font-bold text-slate-800 text-sm mb-3">Retry &amp; Follow-up Queue</h2>
              <div className="space-y-3">
                {[
                  { label: "Calls Requiring Retry", val: "42 Pending Retries", emoji: "🔄", bg: "bg-indigo-50", color: "text-indigo-600" },
                  { label: "Active Retry Queue", val: "18 Dialing Now", emoji: "⏳", bg: "bg-purple-50", color: "text-purple-600" },
                  { label: "Avg. Retry Limit", val: "3 Maximum Attempts", emoji: "📊", bg: "bg-orange-50", color: "text-orange-600" },
                  { label: "Next Scheduled Sweep", val: "In 12 mins (Auto)", emoji: "⏰", bg: "bg-emerald-50", color: "text-emerald-600" }
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className={`text-lg ${item.bg} ${item.color} w-8 h-8 rounded-full flex items-center justify-center font-bold`}>
                      {item.emoji}
                    </span>
                    <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Usage Monitoring */}
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm">
            <h2 className="font-bold text-slate-800 text-sm mb-4">AI Usage &amp; API Budgets</h2>
            
            <div className="grid grid-cols-2 gap-4">
              {[
                { label: "⏱️ Minutes", val: "4,455 / 10k", pct: 44.5, color: "bg-indigo-500" },
                { label: "💬 LLM Tokens", val: "1.2M / 5.0M", pct: 24.0, color: "bg-purple-500" },
                { label: "🔊 TTS Chars", val: "8.4M / 20.0M", pct: 42.0, color: "bg-teal-500" },
                { label: "₹ Monthly Cost", val: "₹1.4k / ₹5.0k", pct: 28.5, color: "bg-rose-500" }
              ].map((item, i) => (
                <div key={i}>
                  ...
                  <p className="text-[10px] text-slate-400 mt-1">{item.pct}% Used</p>
                </div>
              ))}
            </div>

            <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
              <span>Bill Cycle Resets in 12 days</span>
              <button className="text-indigo-600 font-semibold hover:underline">Upgrade Limit</button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="font-bold text-slate-800 text-sm">WhatsApp Escalations</h2>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between py-1.5 border-b border-slate-50">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-slate-600 font-medium">Escalated to WhatsApp</span>
                  </div>
                  <span className="text-xs font-bold text-slate-800">42 Cases</span>
                </div>
                <div className="flex items-center justify-between py-1.5 border-b border-slate-50">
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-blue-500">📥</span>
                    <span className="text-slate-600 font-medium">Client Replies Received</span>
                  </div>
                  <span className="text-xs font-bold text-slate-800">31 Replies</span>
                </div>
                <div className="flex items-center justify-between py-1.5">
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-amber-500">⏳</span>
                    <span className="text-slate-600 font-medium">Awaiting Reply / Pending</span>
                  </div>
                  <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded">11 Pending</span>
                </div>
              </div>
            </div>

            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between items-center">
              <span className="text-[10px] text-slate-400">Response Rate: 73.8%</span>
              <button className="text-xs text-indigo-600 hover:text-indigo-800 font-bold hover:underline">
                Manage Templates
              </button>
            </div>
          ...
          {/* Dynamic Recent Live Activity Feed */}
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-3">
              ...
              <div className="space-y-2.5 max-h-48 overflow-y-auto pr-1">
                {liveActivities.map((act, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs py-1 border-b border-slate-50 transition-all duration-500">
                    <span className="text-slate-400 font-mono w-14 flex-shrink-0">{act.time}</span>
                    <span className="text-slate-600 flex-1 mx-2 truncate font-medium">{act.text}</span>
                    <span className={`shrink-0 ${act.color}`}>{act.status}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px]">
              <span className={`text-slate-400 transition-colors duration-300 ${isSimActiveText ? "text-indigo-600 font-medium" : ""}`}>
                {activitySimText}
              </span>
              <button 
                onClick={simulateEvent} 
                className="btn btn-ghost btn-xs text-indigo-600 hover:text-indigo-800 font-bold p-0 min-h-0 h-auto"
              >
                Trigger Event
              </button>
            </div>
